...
|---|---|---|
| `Minuta_Regulamento_de_Servico_CBMRO_versao_em_consulta.docx` | A minuta como o militar com acesso "Só Regulamento de Serviço" a leu no portal (`/regulamento/servico`, cenário LOB vigente): 7 capítulos, numeração contínua, sem as correções posteriores (estas ficam na versão atual e no Comparativo). Desde 16/09 TODOS os dispositivos suprimidos pela curadoria (16 de 14–15/09, 29 mais um parágrafo de 16/09) saem também desta versão (determinação: o que é suprimido na atual é suprimido na versão em consulta) — 126 artigos. | `node scripts/gerar_docx_regulamento_servico.mjs [--finals .firestore-export/fs_finalTexts.json]` |
| `Residuos_de_outros_CBMs_na_minuta.md` | Levantamento (15/09) das nomenclaturas herdadas do RISD/CBMSE e do Regulamento do CBMMT na minuta publicável — 23 itens, todos aplicados na versão atual (CIOSP→CIOP, SES/SSP→SESAU/SESDEC, "Comandante do SOS"→"Oficial de Dia", Unidade Operacional→OBM, BGO→BG, Centro Integrado de Operações, capítulo da RTO suprimido etc.). | leitura; aplicação em `scripts/regulamento_curadoria_consulta.py` |
| `Transcricoes_da_Lei_2204_nas_sugestoes.md` | Quais dispositivos sugeridos pelo Cel. Luiz Eduardo são transcrição literal da Lei nº 2.204/2009 (Arts. 2º, 35 e 47, § 1º — 60 registros). | leitura |
| `Minuta_Regulamento_de_Servico_CBMRO_parte_geral_e_especial.docx` | **Minuta publicável** (15/09): a versão ATUAL (134 artigos após o lote de 16/09) na estrutura Parte I (Geral, comum aos dois serviços: finalidade e objetivos generalizados para o operacional e o técnico), Parte II (Especial — Título I Serviço Operacional, aberto pela política do serviço operacional; Título II Serviço Técnico, aberto pela política do serviço técnico, artigo novo), Parte III (Finais). Sem autoria da estrutura, sem texto introdutório, sem anexos, sem selos de curadoria; primeira letra de cada frase em maiúscula (`capitalizarFrases`). Artigos reescritos entram no lugar dos antigos, os novos após a âncora, os suprimidos não aparecem. | `node scripts/gerar_docx_regulamento_reestruturado.mjs` |
...
| `Comparativo_Consulta_Regulamento_de_Servico.docx` | Versão em consulta (o texto lido) × versão atual, artigo a artigo (após 16/09: 23 com correção de texto, 28 com texto alterado, 15 reescritos, 6 novos, nenhuma proposta pendente; nenhum dispositivo suprimido aparece, pois as supressões valem para as duas versões). | `node scripts/gerar_docx_regulamento_servico.mjs` (sai junto com a minuta), tela "Comparativo da consulta" ou botão 5 do Pacote |
...
sobre o Bloco 14, que entrou como artigos novos (Superior de Dia, decidido em 15/09: mantido o alcance
estadual; Oficial de Dia, decidido em 15/09: existe só no 1º GBM, na Capital, responsável pelo serviço
operacional diário e pelo serviço interno). Em 15/09 a figura do Comandante de Socorro foi
eliminada por determinação: suas 20 competências passaram ao Oficial de Dia e, nas demais OBMs, ao
Comandante de Guarnição, militar mais antigo de serviço, responsável pelo serviço operacional diário;
também saíram Comandante de Área, Adjunto ao Oficial de Dia, Auxiliares da Guarnição, Guarda de
Quartel e a RTO. A revisão de texto da administração (correções, textos finais, supressões) está
